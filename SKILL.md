...
| Delete task | Delete `tasks/{taskId}.json`, add `metadata/tombstones.json` entry `task-{taskId}`, remove relations in other tasks that target the deleted task (add one relation-deleted activity per removed relation — `relation_deleted`, `relation_block_deleted`, `relation_blockedby_deleted`, or `relation_duplicates_deleted`), and add a `task_deleted` activity. |
